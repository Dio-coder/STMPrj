...
这个函数唯一的作用是遍历 `.init_array` 段里的函数指针，而这个段里会有东西只有两种情况：C++ 全局对象构造函数、C 里标了 `__attribute__((constructor))` 的函数。**纯 C 项目这个段是空的，调不调用结果一样。**

换成官方启动文件后就不行了——它明确写了 `bl __libc_init_array`。关键不在"段是不是空的"，而在**代码有没有去读它**：边界符号正确（start == end）时循环一次都不进就返回；边界符号是垃圾值，空表也会乱跳 → HardFault。
